...
const shirtMeasurementSchema = new Schema(
  {
    userId: {
    ...
    shirtLength: {
      ...
      min: [60, "Shirt length must be at least 60 cm"],
      max: [120, "Shirt length cannot exceed 120 cm"],
    },
    backLength: {
      type: Number,
    ...
    pocketWidth: {
      type: Number,
      min: [5, "Pocket Width size must be at least 5 cm"],
      max: [10, "Pocket Width size cannot exceed 10 cm"],
      default: null,
    },
    pocketHeight: {
      type: Number,
      min: [5, "Pocket Height size must be at least 5 cm"],
      max: [10, "Pocket Height size cannot exceed 10 cm"],
      default: null,
    },
    unit: {
      type: String,
...
shirtMeasurementSchema.index({ userId: 1, profileName: 1 });

shirtMeasurementSchema.pre("save", function (next) {
  // Add logic here for any preprocessing, logging, etc.
  next();
});

const ShirtMeasurement = model("ShirtMeasurement", shirtMeasurementSchema);

module.exports = ShirtMeasurement;
