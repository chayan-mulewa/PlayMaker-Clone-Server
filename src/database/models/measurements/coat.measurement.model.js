...
const coatMeasurementSchema = new Schema(
  {
    userId: {
    ...
    coatLength: {
      type: Number,
      required: [true, "coat length is required"],
      min: [60, "coat length must be at least 60 cm"],
      max: [120, "coat length cannot exceed 120 cm"],
    },
    unit: {
      type: String,
      enum: ["cm", "inches"],
...
coatMeasurementSchema.index({ userId: 1, profileName: 1 });

coatMeasurementSchema.pre("save", function (next) {
  // Add logic here for any preprocessing, logging, etc.
  next();
});

const CoatMeasurement = model("CoatMeasurement", coatMeasurementSchema);

module.exports = CoatMeasurement;
