...
const chinosMeasurementSchema = new Schema(
  {
    userId: {
...
chinosMeasurementSchema.index({ userId: 1, profileName: 1 });

chinosMeasurementSchema.pre("save", function (next) {
  // Add logic here for any preprocessing, logging, etc.
  next();
});

const ChinosMeasurement = model("ChinosMeasurement", chinosMeasurementSchema);

module.exports = ChinosMeasurement;
